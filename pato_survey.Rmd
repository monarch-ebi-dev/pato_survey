...
```{r}
turtle<-function(df,name) {
  #  "suahdius" .

  write.csv(df,paste(name,".csv",sep=""),row.names = FALSE)
  fileConn<-file(paste(name,".ttl",sep=""))
  writeLines(paste("<",df$iri,"> <http://www.purl.obolibrary.org/obo/DEV_",name,"> \"",df$freq,"\"^^xsd:int . ",sep=""), fileConn)
  close(fileConn)
}
```

```{r}
ttl_ax<-unique(da[corpus=="obo20190313",c("iri","o","axiomid")])
ttl_ax$id<-paste(ttl_ax$o,ttl_ax$axiomid,sep = "-")
ttl_ax_id = ttl_ax[,c("id","iri")]
ct_ttl_iri<-count(ttl_ax_id[,c("iri")])

ttl_ax_o<-unique(da[corpus=="obo20190313",c("iri","o")])
ct_ttl_ax_o<-count(ttl_ax_o[,c("iri")])

ttl_exp<-unique(df_exp[corpus=="obo20190313",.(iri,expressionid,axiomid,o)])
...
turtle(ct_ttl_iri,"ct_uses_across_axioms")
turtle(ct_ttl_ax_o,"ct_uses_across_ontologies")
turtle(ct_ttl_exp,"ct_uses_across_expressions")
```
